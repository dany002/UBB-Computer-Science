...
public class MainWindowController {
    ...
    public void refresh(){
        ...
        if(this.controller.getProgStates().size() > 0){
            ...
            this.currentLatchTable = this.controller.getProgStates().get(0).getLatchTable();
        }
        ...
        if(this.currentLatchTable != null)
            this.currentLatchTable.toMap().forEach((x,y) -> this.latchTableTableView.getItems().add(new Pair<>(x,y)));
    ...
    public void initialize(){
        ...
        this.latchAddressColumn.setCellValueFactory(p -> new SimpleIntegerProperty(p.getValue().getKey()).asObject());
        this.latchValueColumn.setCellValueFactory(p -> new SimpleStringProperty(p.getValue().getValue().toString()));
        this.refresh();
