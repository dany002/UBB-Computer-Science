...
class TestStudentTextFileRepository(unittest.TestCase):

    def setUp(self) -> None:
        file_name = "students_text_test"
        self.__student_repository = StudentTextFileRepository(file_name)

    def tearDown(self) -> None:
        if os.path.exists("students_text_test"):
            os.remove("students_text_test")
    """
        TESTS FOR ADD
    """
    def test_add_a_student__with_valid_data__student_is_going_to_be_added_in_the_class(self):
        list_of_expected_students = []
        list_of_expected_students.append(Student(1,"Dani"))
        self.__student_repository.add_a_student(Student(1,"Dani"))
    ...
    def test_add_a_student__raise_exception_RepositoryException__student_id_is_already_taken(self):
        self.__student_repository.add_a_student(Student(2,"Dani"))
        with self.assertRaises(RepositoryException) as re:
            self.__student_repository.add_a_student(Student(2,"Alin"))
    ...
    """
        TESTS FOR LOADING
    """
    def test_load_file__with_valid_data__is_going_to_read_the_elements_from_the_file_that_are_already_written(self):
        self.__student_repository.add_a_student(Student(15, "Dani"))
        second_repository = StudentTextFileRepository("students_text_test")
        self.assertEqual(second_repository.list_of_students(), self.__student_repository.list_of_students())
