...
class TestGradeTextFileRepository(unittest.TestCase):

    def setUp(self) -> None:
        file_name = "grades_text_test"
        self.__grade_repository = GradeTextFileRepository(file_name)

    def tearDown(self) -> None:
        if os.path.exists("grades_text_test"):
            os.remove("grades_text_test")
    ...
    def test_load_file__with_valid_data__is_going_to_read_the_elements_from_the_file_that_are_already_written(self):
        self.__grade_repository.add_a_grade_for_a_student(Grade(3, 5, 10))
        second_repository = GradeTextFileRepository("grades_text_test")
        self.assertEqual(second_repository.list_of_grades(), self.__grade_repository.list_of_grades())
