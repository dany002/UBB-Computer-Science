...
class StudentRepository:
    ...
    def add_a_student(self, student_that_is_going_to_be_added):
        ...
        for index in range(len(self.__students)):
            if self.__students[index].student_id == student_that_is_going_to_be_added.student_id:
                raise RepositoryException("Student ID is not unique.")
        self.__students.append(student_that_is_going_to_be_added)
    ...
    def remove_a_student(self, student_id):
        ...
        id_found = False
        for index in range(len(self.__students)):
            if int(self.__students[index].student_id) == int(student_id):
                id_found = True
        if id_found == False:
    ...
    def search_a_student_by_name(self, student_name):
        ...
        result_list_with_searched_student = []
        for index in range(len(self.__students)):
            if self.__students[index].name == student_name:
                student = {'id': self.__students[index].student_id, 'name': self.__students[index].name}
                result_list_with_searched_student.append(student)
        if result_list_with_searched_student == []:
    ...
    def update_a_student_by_name(self, student_name, new_student_id):
        ...
        """
        for index in range(len(self.__students)):
            if self.__students[index].student_id == int(new_student_id):
                raise RepositoryException("Student ID is already taken.")

        name_found = False
        for index in range(len(self.__students)):
            if self.__students[index].name == student_name:
                self.__students[index].student_id = new_student_id
                name_found = True
        if name_found == False:
    ...
    def update_a_student_by_id(self, student_id, new_student_name):
        ...
        for index in range(len(self.__students)):
            if int(self.__students[index].student_id) == int(student_id):
                self.__students[index].name = new_student_name
                id_found = True
        if id_found == False:
