...
class GradesService:
    ...
    def statistics_for_students_with_best_school_situation(self):
        return sorted(self.all_students_that_have_at_least_a_grade(), key = lambda student: self.compute_aggregated_average_for_a_student(student), reverse = True)
    ...
    def statistics_for_all_disciplines_with_at_least_one_grade(self):
        return sorted(self.all_disciplines_that_have_at_least_a_grade(), key = lambda discipline: self.compute_average_grade_for_a_discipline(discipline), reverse = True)
