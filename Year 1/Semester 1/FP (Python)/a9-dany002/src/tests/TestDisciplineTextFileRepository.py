...
class TestDisciplineTextFileRepository(unittest.TestCase):

    def setUp(self) -> None:
        file_name = "disciplines_text_test"
        self.__discipline_repository = DisciplineTextFileRepository(file_name)

    def tearDown(self) -> None:
        if os.path.exists("disciplines_text_test"):
            os.remove("disciplines_text_test")
    ...
    def test_load_file__with_valid_data__is_going_to_read_the_elements_from_the_file_that_are_already_written(self):
        self.__discipline_repository.add_a_discipline(Discipline(15, "FP"))
        second_repository = DisciplineTextFileRepository("disciplines_text_test")
        self.assertEqual(second_repository.list_of_disciplines(), self.__discipline_repository.list_of_disciplines())
