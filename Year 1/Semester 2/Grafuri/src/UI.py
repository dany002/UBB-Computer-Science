...
class UI:
    ...
    def _menu(self):
        ...
        print("Type 'minimum' to print the minimum path for a graph with costs.")
        print("Type 'activity' to print the earliest and the latest starting time for each activity and the total time of a project")
        print("type 'clique' to find a clique of maximum size.")
    ...
    def get_lowest_length_path(self):
        ...
        try:
            self._directed_graph.BFS(start_vertex, end_vertex)
        except VertexError:
            print("The vertex doesn't exist.")
    ...
    def minimum_cost(self):
        """
        It prints the minimum cost of a walk between 2 vertices.
        :return: None
        """
        start_vertex = int(input("What is the first vertex?"))
        end_vertex = int(input("What is the second vertex?"))
        if self._directed_graph.check_vertex(start_vertex) == False or self._directed_graph.check_vertex(end_vertex) == False:
            print("One of the vertices don't exist.")
            return
        distance, next = self._directed_graph.minimum_walk_with_cost()
        if next[start_vertex][end_vertex] is None:
            print("There is no path between the given verices.")
            return
        path = [start_vertex]
        while start_vertex != end_vertex:
            start_vertex = next[start_vertex][end_vertex]
            path.append(start_vertex)
        print("The path with the minimum cost is:", path)

    def lab_4(self):
        print("Reading graph from lab4_manual_execution...\n")
        self._directed_graph = None
        f = open("lab4_manual_execution.txt")
        for line in f.readlines():
            line = line.split()
            if self._directed_graph is None:
                self._directed_graph = DirectedGraph(list(range(int(line[0]))))
            else:
                if len(line) == 2:
                    self._directed_graph.add_edge(int(line[0]), int(line[1]), 0)
                else:
                    for i in range(len(line)):
                        self._directed_graph.set_activity_duration(i, int(line[i]))
        try:
            self._directed_graph.activities_scheduling()
        except FileError:
            print("The graph is not DAG")

    def read_undirected_graph(self):
        file_name = input("What is the file name? \n")
        self._directed_graph.read_undirected_graph_from_a_file(file_name)
    ...
    def run_console(self):
        ...
        console_or_file = input("Do you want to read from keyboard (Type 'keyboard') or from given input ( Type 'given' ) \n"
                                ...
                                "or an undirected graph ( Type 'undirected' )")
        ...
        if console_or_file == "keyboard":
            ...
            self.create_random_graph()
        elif console_or_file == "undirected":
            self.read_undirected_graph()
        else:
